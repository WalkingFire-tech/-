...
class LayeredMemory:
    ...
    def _init_db(self):
        with sqlite3.connect(self._db_path) as conn:
            ...
            ''')
            for table, col in [('strategic_memory', 'last_updated'), ('procedural_memory', 'last_updated'), ('tool_memory', 'last_used')]:
                conn.execute(f'CREATE INDEX IF NOT EXISTS idx_{table}_updated ON {table}({col})')
    ...
    def sync_strategic_memory(self) -> int:
        ...
        try:
            with sqlite3.connect("data/truths.db", timeout=5) as conn:
                rows = conn.execute(
                ...
                for statement, domain, evidence, level in rows:
                    ...
                    if existing:
                        conn.execute(
                            ...
                            (evidence or 1, min(1.0, (evidence or 1) / 5.0), now, existing[0])
                        )
                    else:
                        conn.execute(
                            ...
                            (f"如何处理{domain or '通用'}问题", statement, f"truth_L{level}",
                             ...
                             min(1.0, (evidence or 1) / 5.0), now, now)
                        )
                    count += 1
        ...
        try:
            with sqlite3.connect("data/trajectory_evolution.db", timeout=5) as conn:
                rows = conn.execute(
                    ...
                    if not existing:
                        conn.execute(
                            ...
                            (query, strategy_desc, "trajectory",
                             ...
                             1, (fitness or 50) / 100.0, (fitness or 50) / 100.0, now, now)
                        )
                        count += 1
        except Exception as e:
    ...
    def sync_procedural_memory(self) -> int:
        ...
        try:
            with sqlite3.connect("data/learning_rules.db", timeout=5) as conn:
                rows = conn.execute(
                ...
                for condition, action, confidence, apply_count, success_count, source in rows:
                    ...
                    if existing:
                        conn.execute(
                            ...
                            (confidence or 0.5, apply_count or 0, success_count or 0, now, existing[0])
                        )
                    else:
                        conn.execute(
                            ...
                            (action, condition, json.dumps([action]), source or "rule",
                             confidence or 0.5, apply_count or 0, success_count or 0, now, now)
                        )
                        count += 1
        except Exception as e:
                    ...
                    if existing:
                        conn.execute(
                            ...
                            (stat.get("calls", 0), int(stat.get("success_rate", 0) * stat.get("calls", 1)),
                             stat.get("success_rate", 0) * 100, now, existing[0])
                        )
                    else:
                        conn.execute(
                            ...
                            (tool_name, stat.get("calls", 0),
                             ...
                             stat.get("success_rate", 0) * 100, now, now)
                        )
                    count += 1
        except Exception as e:
                ...
                if existing:
                    ...
                    conn.execute(
                        ...
                        (new_calls, new_succs, new_avg_q, new_avg_d,
                         json.dumps(params) if params else None, now, eid)
                    )
                else:
                    conn.execute(
                        ...
                        (tool_name, query[:50], json.dumps(params) if params else None,
                         quality, duration_ms, 1, 1 if success else 0, now, now)
                    )
        except Exception as e:
    ...
    def decay_outdated(self) -> Dict:
        ...
        try:
            with sqlite3.connect(self._db_path) as conn:
                ...
                result["strategic_decayed"] = conn.execute(
                ...
                result["procedural_decayed"] = conn.execute(
                ...
                result["tool_decayed"] = conn.execute(
    ...
    def get_stats(self) -> Dict:
        try:
            with sqlite3.connect(self._db_path) as conn:
                strategic = conn.execute("SELECT COUNT(*) FROM strategic_memory").fetchone()[0]
                procedural = conn.execute("SELECT COUNT(*) FROM procedural_memory").fetchone()[0]
                tool = conn.execute("SELECT COUNT(*) FROM tool_memory").fetchone()[0]
                return {
                    "strategic": strategic,
