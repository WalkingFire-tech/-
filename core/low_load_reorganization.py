...
class LowLoadReorganization:
    ...
    def _activate_rules(self) -> dict:
        ...
        try:
            ...
            pending = db.query("SELECT id, confidence, trial_count, trial_success, apply_count, condition, action, source FROM learning_rules WHERE status='pending'")
            ...
            for row in pending:
                ...
                if conf >= 0.6 and trial_cnt >= 3 and trial_succ / max(trial_cnt, 1) >= 0.6:
                    db.execute("UPDATE learning_rules SET status='active', promoted_at=?, promotion_reason='低负载重组自动激活：高置信度+试用期通过' WHERE id=?",
                              (datetime.now().isoformat(), rid), commit=True)
                    result["activated"] += 1
                elif conf >= 0.4 and apply_cnt >= 2:
                    db.execute("UPDATE learning_rules SET status='trial', promoted_at=?, promotion_reason='低负载重组晋升试用：中等置信度+有应用记录' WHERE id=?",
                              (datetime.now().isoformat(), rid), commit=True)
                    result["promoted_to_trial"] += 1
                elif conf >= 0.3 and trial_cnt == 0 and apply_cnt == 0:
                    db.execute("UPDATE learning_rules SET status='trial', promoted_at=?, promotion_reason='低负载重组晋升试用：待验证规则进入试用期' WHERE id=?",
                              (datetime.now().isoformat(), rid), commit=True)
                    result["promoted_to_trial"] += 1
    ...
    def _merge_similar_rules(self) -> dict:
        ...
        try:
            db = self._db("learning_rules.db")
            active = db.query("SELECT id, condition, action, confidence, status FROM learning_rules WHERE status='active'")
    ...
    def _extract_rules_from_experience(self) -> dict:
        ...
        try:
            db_exp = self._db("experience_pool.db")
            high_quality = db_exp.query("SELECT raw_input, response, intent_type FROM experiences WHERE success=1 AND quality_score >= 80 ORDER BY timestamp DESC LIMIT 50")
            ...
            db_rules = self._db("learning_rules.db")
            for raw_input, response, intent_type in high_quality:
                ...
                if not condition or len(condition) < 3:
                    continue
                cr = db_rules.query_one("SELECT COUNT(*) FROM learning_rules WHERE condition LIKE ?", (f"%{condition}%",))
                if cr and cr[0] > 0:
                    continue
                action = response[:200] if response else ""
                db_rules.execute("INSERT INTO learning_rules (condition, action, confidence, status, source, created_at) VALUES (?, ?, ?, 'pending', ?, ?)",
                           (condition, action, 0.3, "experience_extraction", datetime.now().isoformat()), commit=True)
                result["extracted"] += 1
