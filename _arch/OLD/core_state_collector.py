...
class StateCollector:
    ...
    def _init_database(self):
        ...
        if existing_cols and 'layer_name' in existing_cols and 'layer' not in existing_cols:
            db.execute("ALTER TABLE state_reports RENAME COLUMN layer_name TO layer", commit=True)
            logger.info("state_reports: layer_name→layer 迁移完成")
    ...
    def _save_report(self, report: LayerStateReport):
        ...
        try:
            ...
            db.execute('''
                ...
                (layer, status, timestamp, metrics, issues, confidence, report_json)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (
                report.layer_name,
                report.status.value,
